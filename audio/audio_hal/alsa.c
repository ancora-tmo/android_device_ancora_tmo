...
void elem_write(int id,int v,int v1,int v2) {
    ...
    if (ret<0)
        ALOGE("elem_write failed %d", errno);
}
...
int msm_mixer_open(const char *name,int card) {
    ...
    if (ret<0)
        ALOGE("get elist failed %d %d",ret, errno);
    ALOGD("got elist %d", elist.count);
    for (i=0;i<mixercount;i++) {
        ALOGD("id %d name %s: %d %d %d %x",elist.pids[i].numid,elist.pids[i].name, elist.pids[i].device, elist.pids[i].subdevice, elist.pids[i].index ,elist.pids[i].iface);
        einfo.id.numid=elist.pids[i].numid;
        ret=ioctl(mixerfd,SNDRV_CTL_IOCTL_ELEM_INFO,&einfo);
        if (ret<0)
            ALOGE("get einfo failed %d %d",ret, errno);
        ALOGD("einfo type %d %d %d %ld %ld",einfo.type, einfo.access, einfo.count, einfo.value.integer.min, einfo.value.integer.max);
        ...
        ALOGD("eval %ld %ld %ld",elval.value.integer.value[0],elval.value.integer.value[1],elval.value.integer.value[2]);
...
int msm_set_volume(int dec_id, int vol) {
    ALOGD("Set Volume %d %d",dec_id,vol);
